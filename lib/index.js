...
function adaptApiProxy(ctx, apiProxy) {
  ...
  return {
    ...
    async modelCatalog() {
      ...
      if (agentDefaultModel !== void 0 && typeof agentDefaultModel.currentSelection === "function") {
        const sel = agentDefaultModel.currentSelection();
        if (sel && sel.provider && sel.model) {
          fallbackDefault = {
            provider: String(sel.provider),
            model: String(sel.model),
            reasoningEffort: sel.reasoningEffort ? String(sel.reasoningEffort) : void 0
          };
        }
      }
      return {
...
function apply(ctx) {
  ...
  ctx.inject(["connection", "apiProxy"], (child) => {
    if (started) return;
    if (child.get("sessionController") !== void 0) return;
